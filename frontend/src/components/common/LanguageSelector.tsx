          ...
          {languages.map((item) => (
            ...
            >
              <div className="flex items-center gap-2">
                <span className="text-[11px] font-mono text-stone-400 dark:text-zinc-500 font-bold">{item.shortLabel}</span>
                <span>{item.label}</span>
              </div>
              {language === item.code && <Check className="w-3.5 h-3.5 text-emerald-600 dark:text-brand-400" />}
            </button>
          ))}
        </div>
      )}
