  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-4 sm:py-6 pb-24 space-y-6 animate-in fade-in">
      ...
      <div className="relative">
        <Search className="w-4 h-4 text-stone-400 dark:text-zinc-500 absolute left-3.5 top-3.5" />
        <input
          type="text"
          placeholder={t('train.searchPlaceholder')}
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          className="w-full bg-white dark:bg-zinc-900 border border-stone-200 dark:border-zinc-800 rounded-2xl pl-10 pr-10 py-3 text-xs text-stone-900 dark:text-white placeholder-stone-400 dark:placeholder-zinc-500 focus:outline-none focus:border-emerald-500 dark:focus:border-brand-500 transition-colors shadow-xs"
        />
        {searchQuery && (
          <button
            onClick={() => setSearchQuery('')}
            className="absolute right-3.5 top-3.5 text-stone-400 hover:text-stone-700 dark:text-zinc-500 dark:hover:text-white"
          >
            <X className="w-4 h-4" />
          </button>
        )}
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-1.5">
          <span className="text-[11px] font-bold text-stone-500 dark:text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
            <Dumbbell className="w-3.5 h-3.5 text-emerald-600 dark:text-brand-400" />
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all border ${
                    isSelected
                      ? 'bg-emerald-600 dark:bg-brand-500 border-emerald-600 dark:border-brand-500 text-white dark:text-black shadow-xs font-bold'
                      : 'bg-white dark:bg-zinc-900 border-stone-200 dark:border-zinc-800 text-stone-600 dark:text-zinc-400 hover:text-stone-900 dark:hover:text-white hover:border-stone-300 shadow-xs'
                  }`}
                >
                  {t(`equipment.${key}`)}
                </button>
        ...
        <div className="space-y-1.5">
          <span className="text-[11px] font-bold text-stone-500 dark:text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
            <Layers className="w-3.5 h-3.5 text-emerald-600 dark:text-brand-400" />
            ...
            {DIFFICULTY_KEYS.map((key) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all border ${
                    isSelected
                      ? 'bg-stone-900 dark:bg-zinc-100 border-stone-900 dark:border-white text-white dark:text-black shadow-xs font-bold'
                      : 'bg-white dark:bg-zinc-900 border-stone-200 dark:border-zinc-800 text-stone-600 dark:text-zinc-400 hover:text-stone-900 dark:hover:text-white hover:border-stone-300 shadow-xs'
                  }`}
                >
                  {t(`difficulty.${key}`)}
        ...
        <div className="flex items-center justify-between">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="text-xs text-emerald-600 dark:text-brand-400 hover:underline font-medium"
            >
          ...
          <button
            ...
            className={`p-3 rounded-2xl border text-center flex flex-col items-center justify-center gap-1.5 transition-all shadow-xs ${
              selectedCategorySlug === 'all'
                ? 'bg-emerald-600 dark:bg-zinc-800 border-emerald-600 dark:border-brand-500 text-white ring-1 ring-emerald-500/30 dark:ring-brand-500/30 shadow-md font-bold'
                : 'bg-white dark:bg-zinc-900 border-stone-200 dark:border-zinc-800 text-stone-600 dark:text-zinc-400 hover:text-stone-900 dark:hover:text-white hover:bg-stone-50 dark:hover:bg-zinc-850'
            }`}
          >
            <div className="w-8 h-8 rounded-xl bg-stone-100 dark:bg-zinc-950 flex items-center justify-center">
      ...
      <div className="space-y-3 pt-2">
        <div className="flex items-center justify-between">
          ...
          {selectedCategorySlug !== 'all' && (
            <span className="text-xs text-emerald-600 dark:text-brand-400 font-mono">
        ...
        {loading ? (
          <div className="p-12 flex flex-col items-center justify-center text-stone-500 dark:text-zinc-500 space-y-2">
          ...
          <div className="p-8 rounded-3xl bg-white dark:bg-zinc-900 border border-stone-200 dark:border-zinc-800 text-center space-y-3 shadow-xs">
            <p className="text-xs text-stone-600 dark:text-zinc-400">{t('train.noExercises')}</p>
            <button
              onClick={() => {
                setSelectedCategorySlug('all');
                setSelectedEquipment('all');
              ...
              className="px-4 py-2 rounded-xl bg-stone-100 hover:bg-stone-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-xs font-semibold text-stone-900 dark:text-white"
            >
              {t('train.resetFilters')}
            </button>
          </div>
