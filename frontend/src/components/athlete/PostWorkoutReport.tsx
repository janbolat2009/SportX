...
import { CheckCircle2, AlertTriangle, ArrowRight, Share2, Award, Gauge, Activity, Repeat, Clock } from 'lucide-react';
...
export const PostWorkoutReport: React.FC<Props> = ({ session, onClose }) => {
  ...
  const repsList = session.repetitions || [];
  ...
  const getScoreLabel = (val: number) => {
    if (val >= 90) return 'Excellent Technique';
    if (val >= 80) return 'Solid Movement Form';
    if (val >= 70) return 'Minor Adjustments Needed';
    return 'Technique Deviations Detected';
  };
    ...
    <div className="max-w-xl mx-auto bg-surface-card border border-surface-border rounded-3xl p-5 sm:p-7 shadow-2xl animate-in zoom-in-95 duration-200">
      ...
      <div className="text-center pb-6 border-b border-surface-border">
        ...
        <p className="text-xs text-zinc-400 mt-1 flex items-center justify-center gap-2 font-mono">
          ...
          <span>Duration: {Math.round(session.duration_seconds || 0)}s</span>
          <span>•</span>
          <span>Model: {session.model_version || 'sportx-ml-v1.0'}</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 my-6">
        ...
        <div className="p-4 rounded-2xl bg-surface-subtle border border-surface-border text-center flex flex-col items-center justify-center">
          <span className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">Technique Score</span>
        ...
        {/* Volume Card */}
        <div className="p-4 rounded-2xl bg-surface-subtle border border-surface-border text-center flex flex-col items-center justify-center">
          <span className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">Repetitions</span>
          <div className="text-4xl sm:text-5xl font-black text-white tracking-tight my-1">
            {validReps}<span className="text-sm text-zinc-500 font-normal">/{totalReps}</span>
          </div>
          <span className="text-[11px] font-medium text-zinc-400">
            {validReps === totalReps ? 'All reps valid' : `${totalReps - validReps} incomplete ROM`}
          </span>
        </div>
      ...
      {session.issues && session.issues.length > 0 ? (
        <div className="mb-6 space-y-2">
          <h4 className="text-xs font-bold text-zinc-300 uppercase tracking-wider px-1">Primary Technique Cues</h4>
        ...
        <div className="mb-6 p-3.5 rounded-xl bg-brand-500/10 border border-brand-500/20 flex items-center gap-3">
          ...
          <div className="text-left">
            <p className="text-xs font-semibold text-brand-200">Optimal Form Maintained</p>
            <p className="text-xs text-brand-400/80">Joint angles, symmetry, and descent cadence met target benchmarks throughout the set.</p>
          </div>
        </div>
      )}
      ...
      >
        <span>Save to Progress & Finish</span>
